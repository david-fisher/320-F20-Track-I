...
//    }
    ...
    public List<StudentsIn> getStudentsinCourse(@PathVariable(value = "c_id") int c_id) throws Exception {
        List<StudentsIn> studentsInList = studentsInRepository.findByCourseID(c_id);
        return studentsInList;
    }

    @GetMapping ("getCoursesOfStudent/{s_id}")
    public List<StudentsIn> getCoursesOfStudent(@PathVariable("s_id") int s_id) throws Exception {
    	 List<StudentsIn> CoursesOfStudent = studentsInRepository.findByStudentID(s_id);
